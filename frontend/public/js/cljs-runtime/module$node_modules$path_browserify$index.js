shadow$provide.module$node_modules$path_browserify$index = function(global, require, module, exports) {
  ...
  exports.dirname = function(path) {
    ...
    for (var code = path.charCodeAt(0), hasRoot = 47 === code, end = -1, matchedSlash = !0, i = path.length - 1; 1 <= i; --i) {
      if (code = path.charCodeAt(i), 47 === code) {
        if (!matchedSlash) {
          end = i;
          break;
        }
  ...
  exports.basename = function(path, ext) {
    ...
    path = -1 === end ? "" : path.slice(start, end);
    ext && path.substr(-1 * ext.length) === ext && (path = path.substr(0, path.length - ext.length));
    return path;
  };
  exports.extname = function(path) {
    "string" !== typeof path && (path += "");
    for (var startDot = -1, startPart = 0, end = -1, matchedSlash = !0, preDotState = 0, i = path.length - 1; 0 <= i; --i) {
  ...
  var substr = "b" === "ab".substr(-1) ? function(str, start, len) {
    ...
    0 > start && (start = str.length + start);
    return str.substr(start, len);
  };
};

//# sourceMappingURL=module$node_modules$path_browserify$index.js.map
